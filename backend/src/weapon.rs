...
//TODO: maybe do a character state for information like boon level and dist from enemy or even
//enemy state
...
impl Weapon {
    ...
    pub fn dps_no_reload(&self, state: &WeaponState) -> f64 {
        self.current_damage(state) * self.current_fire_rate(state) * self.falloff_damage(state)
    }
    pub fn falloff_damage(&self,  state: &WeaponState) -> f64 {
        ...
        let min_mult = 0.1;
        let max_mult = 1.0;
        if state.dist < min_falloff {
            max_mult
        } else if state.dist > max_falloff {
            min_mult
        } else {
            let range = max_mult - min_mult;
            let max_damage = 1.0;
            let factor = (state.dist - min_falloff) / (max_falloff - min_falloff);
...
mod tests {
    ...
    fn weapon_state_default() {
        ...
        assert_relative_eq!(state.dist, 0.0)
    }
    #[test]
    fn weapon_state_new() {
        let state = WeaponState::new(5, 50, 100, 0, 0, 10.0);
        ...
        assert_relative_eq!(state.dist, 10.0)
    }

    #[test]
    #[should_panic]
    fn weapon_state_new_boon_panic() {
        let _state = WeaponState::new(-1, 10, 10, 10, 0, 0.0);
    }
    ...
    fn weapon_falloff_damage() {
        let weapon = Weapon::new(10.0, 0.5, 2.5, 2.0, 15, WeaponType::Auto, 12.0, 24.0);
        let state = WeaponState{dist:16.0, ..WeaponState::default()};
        assert_relative_eq!(weapon.falloff_damage( &state), 0.7);
    }

    #[test]
    fn weapon_min_falloff() {
        let weapon = Weapon::new(10.0, 0.5, 2.5, 2.0, 15, WeaponType::Auto, 12.0, 24.0);
        let state = WeaponState{dist:5.0, ..WeaponState::default()};
        assert_relative_eq!(weapon.falloff_damage( &state), 1.0);
    }

    #[test]
    fn weapon_max_falloff() {
        let weapon = Weapon::new(10.0, 0.5, 2.5, 2.0, 15, WeaponType::Auto, 12.0, 24.0);
        let state = WeaponState{dist:32.0, ..WeaponState::default()};
        assert_relative_eq!(weapon.falloff_damage( &state), 0.1)
    }
    #[test]
